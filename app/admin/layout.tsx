...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 relative overflow-hidden">
      ...
      <div className={`
        fixed inset-y-0 left-0 z-40 bg-white border-r border-slate-100 transition-all duration-300 ease-in-out h-screen shrink-0
      ...
      `}>
        ...
        {/* HEADER TOPBAR */}
        <header className="h-16 bg-white border-b border-slate-100 flex items-center px-4 lg:px-8 shrink-0 justify-between">
          <button
            onClick={() => setSidebarOpen(!sidebarOpen)}
            className="p-2 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-600 transition lg:hidden flex items-center justify-center"
            title="Buka Menu"
          >
        ...
        {/* TEMPAT RENDERING KONTEN HALAMAN */}
